...
success = 0
errors = 0
timestart = __import__("time").time()
console = MPrint()
class Botter(httpx.Client):
    def __init__(self, rawInvite: str):
        ...
        super().__init__(headers={
            ...
            "TE": "trailers"
        }, proxies=self.utils.proxy, timeout=60)
        self.rawInvite = rawInvite
        self.get("https://discord.com") # Get cookies
        self.headers["X-Fingerprint"] = self.get("https://discord.com/api/v9/experiments").json().get("fingerprint")
    ...
    def generateToken(self):
        ...
        global timestart
        try:
            os.system(f"title Shahzain Botter - Success: {success} - Proxies: {len(open('input/proxies.txt').readlines())} - Captcha Balance: ${self.captcha.getBalance()} Erros: {errors}")
            payload = {"fingerprint":self.headers.get("X-Fingerprint"),"username":self.utils.username,"invite":self.rawInvite,"consent":True,"gift_code_sku_id":None,"captcha_key":None}
